...
""")
    ...
    def convert_number_to_words(self, number):
        ...
        try:
            ...
            return number
    def replace_short_words(self, text):
        """
        Заменяет сокращенные слова на полные
        """
        short_words_mapping = {
            'см': 'сантиметр',
        ...
        for short, full in short_words_mapping.items():
            text = text.replace(short, full)
        return text

    def inflect_word(self, word):
        """
        Склоняет слово с использованием pymorphy
        """
        parsed_word = morph.parse(word)[0]
        return parsed_word.inflect({'gent'}).word if parsed_word.tag.gender else word    
    ...
    def save_nlu_to_file(
        ...
        with open(output_filename, 'a') as fout:
            intent_examples = eval(intent_examples.replace('[', '').replace(']', '').replace('\n', ''))

            intent_msgs_str = f"{TAB_SYMBOL * 2}- " + f'\n{TAB_SYMBOL * 2}  - '.join(
                [self.inflect_word(self.replace_short_words(self.convert_number_to_words(msg))) for msg in intent_examples])
            fout.write(f"""- intent: {self.inflect_word(intent_name)}
